...
// opcode(2) + flags(1) + dst(2) + src_count(1) + srcs(2*n).
constexpr size_t meta_offset(size_t instr_offset, size_t src_count) noexcept {
    return instr_offset + 6 + 2 * src_count;
}
}  // namespace
...
[[noreturn]] void asm_fail(uint32_t line, const std::string& msg) {
...
std::vector<Tok> tokenize(const std::string& line, uint32_t line_no) {
    ...
    while (i < line.size()) {
        ...
        if (c == '"') {
            ...
            if (i >= line.size()) asm_fail(line_no, "unterminated string");
        ...
        asm_fail(line_no, std::string("unexpected character '") + c + "'");
    }
    return out;
}

uint16_t parse_register(const Tok& t) {
    if (t.text.size() < 2 || (t.text[0] != 'v' && t.text[0] != 'V')) {
        asm_fail(t.line, "expected register, got '" + t.text + "'");
    }
    const int v = std::atoi(t.text.c_str() + 1);
    if (v < 0 || v > 0xFFFF) asm_fail(t.line, "register out of range");
    return static_cast<uint16_t>(v);
}

int64_t parse_int(const Tok& t) {
    try {
        return static_cast<int64_t>(std::stoll(t.text));
    } catch (...) {
        asm_fail(t.line, "expected integer, got '" + t.text + "'");
    }
}

double parse_float(const Tok& t) {
    try {
        return std::stod(t.text);
    } catch (...) {
        asm_fail(t.line, "expected float, got '" + t.text + "'");
    }
}
...
struct ParserCursor {
    ...
    void expect(const std::string& s) {
        if (eof() || peek().text != s) {
            asm_fail(eof() ? line : peek().line, "expected '" + s + "'");
        }
        ++pos;
    }
};
    ...
    try {
        ...
        std::vector<std::vector<Tok>> lines;
        {
            ...
            while (std::getline(in, raw)) {
                ++line_no;
                auto toks = tokenize(raw, line_no);
        ...
        for (auto& toks : lines) {
            ParserCursor cur{&toks, 0, toks.empty() ? 0 : toks[0].line};
            const Tok& head = cur.peek();

            if (head.text == ".language") {
                cur.next();
                module.language_name = cur.next().text;
                continue;
            }
            if (head.text == ".class") {
                cur.next();
                module.intern_class(cur.next().text);
                continue;
            }
            if (head.text == ".field") {
                cur.next();
                const std::string fname = cur.next().text;
                cur.expect("in");
                const std::string klass = cur.next().text;
                module.intern_field(fname, module.intern_class(klass));
                continue;
            }
            if (head.text == ".builtin") {
                cur.next();
                module.intern_builtin(cur.next().text);
                continue;
            }
            if (head.text == ".method") {
                if (in_method) asm_fail(head.line, "nested .method");
                cur.next();
                ...
                if (cur.take_if("(")) {
                    while (!cur.eof() && cur.peek().text != ")") {
                        ...
                        cur.expect("=");
                        const int64_t v = parse_int(cur.next());
                        if (key == "regs") regs = static_cast<uint16_t>(v);
                        else if (key == "args") args = static_cast<uint16_t>(v);
                        else asm_fail(head.line, "unknown method attribute " + key);
                        cur.take_if(",");
                    }
                    cur.expect(")");
                }
            ...
            if (head.text == ".end") {
                cur.next();
                if (!in_method || !builder) asm_fail(head.line, ".end without .method");
            ...
            if (cur.peek().text != "" && cur.pos + 1 < toks.size() &&
                toks[cur.pos + 1].text == ":") {
                if (!in_method) asm_fail(head.line, "label outside .method");
            ...
            // Instruction.
            if (!in_method) asm_fail(head.line, "instruction outside .method");
            const std::string op_text = cur.next().text;
            const Op op = parse_opcode(op_text);
            if (op == Op::ILLEGAL) {
                asm_fail(head.line, "unknown opcode '" + op_text + "'");
            }
            ...
            if (!cur.eof()) {
                ...
                while (cur.take_if(",")) ops.push_back(cur.next());
            }

            auto need = [&](size_t n) {
                if (ops.size() != n) {
                    asm_fail(head.line, "opcode " + op_text + " expects " +
                                            std::to_string(n) + " operands, got " +
                                            std::to_string(ops.size()));
                }
            };
            auto reg = [&](size_t i) { return parse_register(ops[i]); };
            auto label_ref = [&](const std::string& lbl, size_t offset) {
                // Patch after finish(): reuse MethodBuilder's label mechanism by
                // recording through the public API below.
                (void)lbl;
                (void)offset;
            };
            ...
            case Op::CONST_I32:
                need(2);
                off = builder->const_i32(reg(0),
                                         static_cast<int32_t>(parse_int(ops[1])));
                break;
            case Op::CONST_I64:
                need(2);
                off = builder->const_i64(reg(0), parse_int(ops[1]));
                break;
            case Op::CONST_F64:
                need(2);
                off = builder->const_f64(reg(0), parse_float(ops[1]));
                break;
            case Op::CONST_STRING:
                ...
                break;
            case Op::CONST_NULL: need(1); off = builder->const_null(reg(0)); break;
            case Op::CONST_TRUE: need(1); off = builder->const_true(reg(0)); break;
            case Op::CONST_FALSE: need(1); off = builder->const_false(reg(0)); break;
            case Op::MOVE: need(2); off = builder->emit(op, reg(0), {reg(1)}); break;
            ...
            case Op::EQ_ANY: case Op::COMPARE_ANY:
                need(3);
                off = builder->emit(op, reg(0), {reg(1), reg(2)});
                break;
            ...
            case Op::BOX: case Op::UNBOX: case Op::ANY_TO_TYPED: case Op::TYPED_TO_ANY:
                need(2);
                off = builder->emit(op, reg(0), {reg(1)});
                break;
            case Op::RETURN:
                need(1);
                off = builder->ret(reg(0));
                break;
            ...
            case Op::CALL_BUILTIN:
                need(4);
                off = builder->call_builtin(reg(0), reg(1),
                                            static_cast<uint16_t>(parse_int(ops[2])),
                                            ops[3].text);
                break;
            case Op::NEW_OBJECT:
            ...
            case Op::ARRAY_SET: case Op::ARRAY_SET_UNCHECKED:
                ...
                off = builder->emit(op, 0xFFFF, {reg(0), reg(1), reg(2)});
                break;
            case Op::CHECK_NULL: case Op::CHECK_NON_NULL:
                need(1);
                off = builder->emit(op, 0, {reg(0)});
                break;
            ...
            case Op::CHECK_BOUNDS:
                need(2);
                off = builder->emit(op, 0, {reg(0), reg(1)});
                break;
            case Op::DEBUG_SRCPOS:
                need(1);
                off = builder->emit(op, 0, {}, true,
                                    static_cast<uint32_t>(parse_int(ops[0])));
                break;
            case Op::DEBUG_TRAP:
                need(0);
                off = builder->emit(op);
            ...
            case Op::EXT_OP:
                ...
                off = builder->emit(op, reg(0), {reg(1)}, true,
                                    static_cast<uint32_t>(parse_int(ops[1])));
                break;
            default:
                asm_fail(head.line, "opcode not supported by the M0 assembler: " +
                                        op_text);
            }
            (void)off;
            (void)label_ref;
        ...
        return support::fail(support::ErrorCode::AssembleError, e.what());
    }
    return module;
}
